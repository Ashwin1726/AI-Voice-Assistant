...
class AesunAI:
    def __init__(self, gemini_api_key: str, groq_api_key: str, data_dir: str = "Aesun_data"):
        """
        Initialize Aesun AI Assistant with Agentic capabilities
        ...
        # Wake word detection
        self.wake_word = "Aesun"
        self.sleep_command = "Aesun go back to sleep"
        ...
        logger.info("Aesun AI Assistant initialized successfully!")
        
        print("AesunAI Initialized")
    ...
    def generate_response_with_context(self, user_input: str) -> str:
        ...
        try:
            ...
            # --- Combine all context ---
            prompt = f"""
            You are Aesun, a helpful AI assistant. Use the following context to provide personalized responses.
    ...
    def process_user_input(self, user_input: str) -> str:
        ...
        if self.sleep_command in user_input.lower():
            self.is_active = False
            self.listening_for_wake_word = True
            return "Going back to sleep mode. Say 'Aesun' to wake me up."
    ...
    def run_continuous_mode(self):
        """Run Aesun in continuous voice mode with wake word detection"""
        print("Aesun is now running in continuous mode...")
            ...
            while True:
                if self.listening_for_wake_word:
                    ...
                    if user_input:
                        ...
                        print(f"Aesun: {response}")
                        self.speak(response)
                        
                        # Check if going back to sleep
                        if not self.is_active:
                            print("Aesun is now sleeping. Say 'Aesun' to wake up.")
                    else:
                        # Timeout - ask if user is still there
                        print("No input detected. Going back to sleep mode.")
                        self.speak("I'll go back to sleep now. Say Aesun to wake me up.")
                        self.is_active = False
                        self.listening_for_wake_word = True
    ...
    def run_interactive_mode(self):
        """Run Aesun in text-based interactive mode"""
        print("Aesun Interactive Mode - Type 'exit' to quit")
        print("Aesun: Hello! I'm Aesun, your AI assistant. How can I help you today?")
        
        while True:
            user_input = input("You: ").strip()
            
            if user_input.lower() in ['exit', 'quit', 'goodbye']:
                print("Aesun: Goodbye! Have a great day!")
                break
...
if __name__ == "__main__":
    # Initialize Aesun with your API keys
    ...
    try:
        ...
        if choice == "1":
            ...
            Aesun.run_interactive_mode()
            
    except Exception as e:
        print(f"Error initializing Aesun: {e}")
